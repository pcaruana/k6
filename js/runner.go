...
func (r *Runner) newVU(idLocal, idGlobal uint64, samplesOut chan<- stats.SampleContainer) (*VU, error) {
	...
	moduleVUImpl := &moduleVUImpl{ctxPtr: new(context.Context), eventLoop: newEventLoop()}
...
func (u *VU) runFn(
	...
	// TODO: we can optimize this to only make a new one if the previous one errored out
	u.moduleVUImpl.eventLoop = newEventLoop()
	u.moduleVUImpl.eventLoop.initHelpers(ctx, u.Runtime)
	...
	if isDefault && err != nil {
		cancel()
		u.moduleVUImpl.eventLoop.waitOnReserved()
	}
	endTime := time.Now()
